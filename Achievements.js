...
const modifiers=['standard','blitz','fortified','volatile','bounty','glassstorm','nightfall','saprush','bossmarch','ricochet'];
const difficulties=['scout','uprising','veteran','mayhem','apocalypse','nightmare','legendary'];
...
const defs=[
 ...
 {id:'fighter_collection_half',tier:'standard',icon:'🐿️',name:'Burrow Roll Call',desc:'Unlock at least half of the fighter roster.',target:5,value:m=>(m.unlockedFighters||[]).length},
 {id:'fighter_collection_all',tier:'legendary',icon:'🎭',name:'Every Tail Accounted For',desc:'Unlock every fighter.',target:fighters.length,value:m=>(m.unlockedFighters||[]).length},
 {id:'map_collection_half',tier:'standard',icon:'🧭',name:'Trailblazer',desc:'Unlock at least half of the arenas.',target:3,value:m=>(m.unlockedMaps||[]).length},
 {id:'map_collection_all',tier:'legendary',icon:'🗺️',name:'Canopy Cartographer',desc:'Unlock every arena.',target:arenas.length,value:m=>(m.unlockedMaps||[]).length},
 {id:'relic_collection_half',tier:'standard',icon:'💍',name:'Half-Forged Legend',desc:'Forge at least half of the 90 relic blueprints.',target:45,value:m=>(m.relics||[]).length},
 {id:'relic_collection_all',tier:'legendary',icon:'👑',name:'Relic Sovereign',desc:'Forge all 90 relic blueprints.',target:90,value:m=>(m.relics||[]).length},
 {id:'modifier_collection_half',tier:'standard',icon:'🧬',name:'Rule Breaker',desc:'Unlock at least half of the run modifiers.',target:5,value:m=>(m.unlockedModifiers||[]).length},
 {id:'modifier_collection_all',tier:'legendary',icon:'🎛️',name:'Master of Conditions',desc:'Unlock every run modifier.',target:modifiers.length,value:m=>(m.unlockedModifiers||[]).length},
 {id:'difficulty_collection_half',tier:'standard',icon:'⚔️',name:'Rising Threat',desc:'Unlock four difficulty tiers.',target:4,value:m=>(m.unlockedDifficulties||[]).length},
 {id:'difficulty_collection_all',tier:'legendary',icon:'🏆',name:'Canopy Legend Cleared',desc:'Unlock every difficulty tier.',target:difficulties.length,value:m=>(m.unlockedDifficulties||[]).length},
...
function normalize(meta={}){const oldAcorns=Number(meta.totalAcorns)||0,playedFighters=Object.keys(meta.fightersPlayed||{}),playedMaps=Object.keys(meta.arenasPlayed||{}),fighterUnlocks=Array.isArray(meta.unlockedFighters)?meta.unlockedFighters:playedFighters,mapUnlocks=Array.isArray(meta.unlockedMaps)?meta.unlockedMaps:playedMaps,modifierUnlocks=Array.isArray(meta.unlockedModifiers)?meta.unlockedModifiers:[],difficultyUnlocks=Array.isArray(meta.unlockedDifficulties)?meta.unlockedDifficulties:[];return{...meta,bestWave:Number(meta.bestWave)||0,totalAcorns:oldAcorns,totalSap:Number(meta.totalSap)||0,lifetimeSap:Number(meta.lifetimeSap)||0,relics:Array.isArray(meta.relics)?meta.relics:[],equippedRelics:meta.equippedRelics||{chest:null,helmet:null,leggings:null,boots:null,amulet:null,ring1:null,ring2:null},lifetimeAcorns:meta.lifetimeAcorns==null?oldAcorns:Number(meta.lifetimeAcorns)||0,runs:Number(meta.runs)||0,lifetimeKills:Number(meta.lifetimeKills)||0,bossesDefeated:Number(meta.bossesDefeated)||0,highestCombo:Number(meta.highestCombo)||0,pickupsCollected:Number(meta.pickupsCollected)||0,totalSurvivalSeconds:Number(meta.totalSurvivalSeconds)||0,flawlessWaves:Number(meta.flawlessWaves)||0,flawlessBosses:Number(meta.flawlessBosses)||0,upgradesChosen:Number(meta.upgradesChosen)||0,largestRunAcorns:Number(meta.largestRunAcorns)||0,maxKillsRun:Number(meta.maxKillsRun)||0,arenasPlayed:meta.arenasPlayed||{},fightersPlayed:meta.fightersPlayed||{},difficultyBest:meta.difficultyBest||{},armory:meta.armory||{},unlockedAchievements:Array.isArray(meta.unlockedAchievements)?meta.unlockedAchievements:[],unlockedFighters:[...new Set(['rookie',...fighterUnlocks.filter(id=>fighters.includes(id))])],unlockedMaps:[...new Set(['oak',...mapUnlocks.filter(id=>arenas.includes(id))])],unlockedModifiers:[...new Set(['standard',...modifierUnlocks.filter(id=>modifiers.includes(id))])],unlockedDifficulties:[...new Set(['scout','uprising',...difficultyUnlocks.filter(id=>difficulties.includes(id))])]};}
...
window.ENAchievements={defs,normalize,progress,evaluate,formatValue,fighters,arenas,modifiers,difficulties};
})();
